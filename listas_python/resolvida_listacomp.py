...
def produto_cartesiano_filtro(homens, mulheres):
    ...
    return [(h, m) for h in homens for m in mulheres if len(h)>=4 and len(m) >=4]
...
def pares_e_divisiveis_por_7(limite_inicial=1067, limite_final=3627):
    ...
    return len([x for x in range(1067,3628) if x % 2 == 0 and x % 7 == 0])
